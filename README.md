...
- Open Source3D Stream Audio — Positional streaming audio from prims with multi-speaker / 5.1ch support
  ...
  - **Reichweite:** `[3dstream:{url:...}{range:30}]` (default 20m)
  - **Distributed Stereo** (mehrere Speaker im Linkset):
    - Root-Prim: `[3dstream:{url:http://example.com/stream.mp3}]`
    - Speaker: `[3dstream:{ch:L}{range:30}]` / `[3dstream:{ch:R}{range:30}]`
  - **Optionale Parameter:** `binaural:on|off`, `venue:NAME` (hall_medium, club, cathedral...), `upmix:on|off`, `volume:0.8`
  - **Aktivierung:** Preferences > Sound > `3D Stream Enabled` anhaken, Volume regeln
