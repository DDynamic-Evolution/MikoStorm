...
{
	// Update check not applicable for MikoStorm native integration
}
...
bool reshade::runtime::load_effect(const std::filesystem::path &source_file, const ini_file &preset, size_t effect_index, size_t permutation_index, bool force_load, bool preprocess_required)
{
	...
	attributes += "version=" + std::to_string(RESHADE_VERSION_MAJOR * 10000 + RESHADE_VERSION_MINOR * 100 + RESHADE_VERSION_REVISION) + ';';
	...
	if (!compiled && !source.empty())
	{
		...
		{
#if defined(RESHADE_D3D9) || defined(RESHADE_D3D10) || defined(RESHADE_D3D11) || defined(RESHADE_D3D12)
