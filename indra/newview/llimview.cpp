...
extern void pandaview_flash_ll_im_container(const LLSD& msg);
extern void pandaview_show_ll_im_conversation(const LLUUID& session_id);
// </FS:AYA>
    ...
    if (gSavedSettings.getS32("NotificationToastLifeTime") > 0 || gSavedSettings.getS32("ToastFadingTime") > 0) // Ansa: only create toast if it should be visible at all
    {
        ...
        if (pandaview_is_ll_style())
        {
            ...
            LLNotificationsUtil::add("IMToast", args, LLSD(), [session_id](const LLSD&, const LLSD&) {
                pandaview_show_ll_im_conversation(session_id);
            });
        }
        else
        {
...
LLIMModel::LLIMModel()
{
    ...
    addNewMsgCallback(boost::bind(&LLFloaterIMSession::newIMCallback, _1));
    addNewMsgCallback(boost::bind(&pandaview_flash_ll_im_container, _1));
    // </FS:AYA>
    ...
    bool keyword_alert_performed) // <FS:Ansariel> Pass info if keyword alert has been performed
{
    ...
    {
        // <FS:AYA> Phase 3: Route to LL or FS based on AYAChatWindowStyle
        LLFloaterReg::showInstance(pandaview_im_container_name());
        // </FS:AYA>
    // </FS:CR>
    }
}
